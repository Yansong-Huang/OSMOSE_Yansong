...
run_model = function(par, conf, osmose, is_a_test=FALSE, version="4.3.3", options="-Xmx3g -Xms1g", ...) {
  ...
  catchability.fsh3.sp1 = get_par(conf, "osmose.user.yansong.par1")
  # 2. read the catchability matrix
  catmat = read.csv(file="eec_fisheries_catchability.csv", check.names = FALSE, row.names = 1)
  # 3. modify the catchability matrix
  catmat[1,3] = catchability.fsh3.sp1
  # 4. write the new modified catchability matrix
  write_osmose(catmat, file="eec_fisheries_catchability.csv") # better than write.csv
