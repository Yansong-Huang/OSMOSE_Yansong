...
# ----------------------------------------------
# 计算每个风电场区域内的总生物量相对变化
# Compute the relative total biomass inside each OWF area
...
library(purrr)
library(ncdf4)
source("scripts_analysis/OWF_locations.R")  # 定义风电场位置
...
OWF_names <- c("Calvados", "Fecamp", "Dieppe", "Dunkerque", 
               "Centre Manche 1", "Centre Manche 2", 
               "Rampion", "Future OWF")

colour_palette <- c(rep("darkred", 4), rep("darkgreen", 3), rep("darkblue", 2))

# 指定需要比较的情景 / Define scenario to compare
regulation <- regulation_scenarios[1]
results_path_base <- file.path("outputs/results_2510", "Base_simu", "output", "CIEM")
results_path_OWF <- file.path("outputs/results_2510",
                              paste0("CC.", CC_scenarios[1], "_", deployment_scenarios[4], "_", regulation),
                              "Base", "output", "CIEM")
...
process_biomass <- function(current_results_path) {
  ...
  for (replicate in 1:n_replicate) {
    ...
    for (OWF_name in names(OWF_locations)) {
      ...
      biomass_spatial_OWF_base <- aperm(simplify2array(biomass_spatial_OWF_base_list), c(3, 1, 2))
      biomass_spatial_OWF_current <- aperm(simplify2array(biomass_spatial_OWF_current_list), c(3, 1, 2))
  ...
  for (OWF_name in names(relative_biomass_by_OWF_all_replicates)) {
    ...
    relative_biomass_by_OWF_summary[[OWF_name]] <- data.frame(
      year = years,
      mean = rowMeans(yearly_data),
      sd = apply(yearly_data, 1, sd)
    )
  }
...
biomass_plot <- ggplot(relative_biomass_combined) +
  ...
  # 三组风电场的注释时间窗口 / Highlight different OWF operation periods
...
# 保存图像 / Save plot
ggsave(file.path("figures/publication/time_series", regulation, "total_biomass_by_OWF.png"),
       biomass_plot, width = 15, height = 8, dpi = 600)
