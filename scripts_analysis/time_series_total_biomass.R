...
cut_off_year <- 10 
n_replicate <- 30
biomass_sd_colour_palette <- c("#8a2be2","#0892d0","#ff3800","#ff1493")
biomass_mean_colour_palette <- c("#8a2be2","#0892d0","#ff3800","#ff1493")
...
combined_plot <- ggplot(combined_biomass_ratio_df) +
  ...
  scale_color_manual(name = "Deployment Scenario", values = biomass_mean_colour_palette) +
  scale_fill_manual(name = "Deployment Scenario", values = biomass_sd_colour_palette) +
  ylab("Biomass Ratio") +
...
# 保存合并后的图像
ggsave("figures/publication/time_series/combined_biomass_vertical.png", combined_plot, width = 8, height = 8, dpi = 600)
