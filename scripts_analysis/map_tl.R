...
process_maps <- function(species_group,species_group_name,n_years_begin, n_years_end){
  map_base_list <- lapply(1:10, function(simulation){
    ...
    #read variable
    TL_spatial_base <- ncvar_get(TL_nc_base,"TL")
    nc_close(TL_nc_base)
    
    # cut from one period
    TL_spatial_base <- TL_spatial_base[,,species_group,n_years_begin:n_years_end] # to be replaced
    
    # sum up all species
    TL_total_base <- apply(TL_spatial_base, c(1,2), sum)
    
    return(TL_total_base)
  })
  ...
  map_current_list <- lapply(1:10, function(simulation){
    ...
    #read variable
    TL_spatial_current <- ncvar_get(TL_nc_current,"TL")
    nc_close(TL_nc_current)
    
    # cut from one period
    TL_spatial_current <- TL_spatial_current[,,species_group,n_years_begin:n_years_end]
    
    # sum up all species
    TL_total_current <- apply(TL_spatial_current, c(1,2), sum)
    
    return(TL_total_current)
  })
  ...
  ratio_map_plot <- ggplot() +
    ...
    labs(title = paste0(n_years_begin+2001,"-",n_years_end+2001," ",species_group_name," species TL change"), x = "Lon", y = "Lat") +
    theme_bw()
  print(ratio_map_plot)
  ggsave(file.path("figures_CIEM_CC_old", deployment, regulation, "TL_map",
                   paste0(species_group_name,"_",n_years_begin, "_mean_TL_map.png")),
...
for (reg in 1:length(regulation_scenarios)){
  ...
  list_TL_nc_current <- list.files(current_results_path, "Yansong_spatializedTL_Simu.", full.names = TRUE)
