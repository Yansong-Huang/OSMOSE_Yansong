...
process_LFI <- function(current_results_path, cut_off_year_begin, cut_off_year_end) {
  # 获取文件列表
  list_biomass_base <- list.files(results_path_base, "Yansong_biomassDistribBySize_Simu.*csv", full.names = TRUE)
  list_biomass_current <- list.files(current_results_path, "Yansong_biomassDistribBySize_Simu.*csv", full.names = TRUE)
...
# 保存图像
ggsave(
  file.path("figures", "publication", "boxplot", "LFI_combined.png"),
  combined_boxplot,
